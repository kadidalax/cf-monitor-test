...
<ul>
<li><b>修复</b> 流量统计把同一份流量算了两遍，面板数值远高于商家计量。默认只统计承载默认路由的网卡，隧道与 VPN（WireGuard、tailscale、WARP 等）、容器与桥接网卡不再计入——数据离开机器必然经过物理网卡，那里已经算过一次。累计流量与网速改用同一套网卡。实测某节点上报值为实际出网量的 <code>1.97</code> 倍，修复后偏差 <code>0.02%</code>。多公网口分走不同线路的机器可用 <code>--nic-include</code> 手动指定。<b>需重新安装或升级 Agent 后生效。</b></li>
...
<li><b>变更</b> 历史写入熔断改按数据库的真实磁盘占用（含索引）判定，新增「历史写入熔断容量」默认 400 MiB；原「熔断行数」保留为次要边界，两者谁先到谁生效。同样的行数可能占 72MB 也可能 189MB，按行数判定落不到真正快满的地方。</li>
